...
public:
    ...
    void ExamineVertex(const Vertex &vertex) {
        v->last_poped = vertex;
        v->distance = v->vertexs[v->last_poped].second + 1;
    }

    void DiscoverVertex(const Vertex &vertex) {
        if (vertex == 0) {
            v->vertexs.insert(std::make_pair(0, std::make_pair(0, 0)));
        } else if (v->vertexs.find(vertex) != v->vertexs.end()) {
            if (v->distance < v->vertexs[vertex].second) {
                v->vertexs[vertex].first = v->last_poped;
                v->vertexs[vertex].second = v->distance;
            }
        } else {
            v->vertexs.insert(std::make_pair(vertex, std::make_pair(v->last_poped, v->distance)));
        }
    }

    size_t DistanceTo(const Vertex &target) const {
        return v->vertexs[target].second;
    }

    Vertex Parent(const Vertex &vertex) const {
        return v->vertexs[vertex].first;
    }

private:
    class Vert {
    public:
        std::map<int, std::pair<int, int>> vertexs;
        int last_poped;
        int distance = 0;
    };
