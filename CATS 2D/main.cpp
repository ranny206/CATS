...
class FormatVisitor: public BaseVisitor {
 public:
  ...
  void Visit(const ClassDeclarationNode* node) override {
    result.push_back("class" + node->ClassName() + " " + "{");
    indent++;
    std::string s = Get_indent(indent);
    if(node->PrivateFields().size() != 0){
      result.push_back(s + "public:");
      for (auto i : node->PublicFields()) {
        i->Visit(this);
      }
    }
    if(node->ProtectedFields().size() != 0){
      result.push_back(s + "protected:");
      for (auto i : node->ProtectedFields()) {
        i->Visit(this);
      }
    }
    if(node->PrivateFields().size() != 0){
      result.push_back(s + "private:");
